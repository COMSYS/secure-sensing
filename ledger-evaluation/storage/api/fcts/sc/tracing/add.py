...
"""
Add Tracing Information of a specific Produce Entry. Keys are chosen by the Storage.
"""
def run(args):
    res = args["api"].res
    api = args["api"]
    ...
    for k in updates:
        ...
        while npath in tracing:
            npath = pref + uuid.uuid4().hex
        update["path"] = npath
        i = i + 1

    try:
        set = v.updateRecord(doc, updates, api.auth.address, "_tracing", versionSig)
